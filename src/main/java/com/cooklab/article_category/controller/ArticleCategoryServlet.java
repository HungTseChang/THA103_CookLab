...
//			Integer articleCategoryNo = Integer.valueOf(req.getParameter("articleNo").trim());
		...
		if ("cateSearch".equals(action)) { 
			...
			/*************************** 1.接收請求參數 - 輸入格式的錯誤處理 **********************/
			String str = req.getParameter("articleCategoryNo");
			...
			Integer articleCategoryNo = null;
			try {
				articleCategoryNo = Integer.valueOf(str);
			} catch (Exception e) {
			...
			/*************************** 2.開始查詢資料 *****************************************/
			ArticleCategoryService artSvc = new ArticleCategoryService();
			ArticleCategoryVO artCate = artSvc.getOne(articleCategoryNo);
			if (artCate == null) {
				errorMsgs.add("查無資料");
			}
			// Send the use back to the form, if there were errors
